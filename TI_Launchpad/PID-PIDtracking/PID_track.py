...
while True:
    ...
    # Ball tracking and data exchange
    for contour in contours:
        ...
        if cv2.contourArea(contour) > 500:    # ignore detected areas that are too small
            ...
            # Reference position
            x_ref = round(x_center_traj + R_traj * np.cos((2 * np.pi / T_traj) * time_py))
            y_ref = round(y_center_traj + R_traj * np.sin((2 * np.pi / T_traj) * time_py))
            x_ref_print = round((612-33)/450*x_center_traj + 323 + (612-33)/450*R_traj * np.cos((2 * np.pi / T_traj) * time_py))
            y_ref_print = round((467-17)/350*y_center_traj + 242 + (467-17)/350*R_traj * np.sin((2 * np.pi / T_traj) * time_py))

            cv2.circle(frame, (x_ref_print, y_ref_print), 5, (0, 255, 255), -1)
